...
Pipeline completo:
    1. Carga hiperparámetros desde expert1_config.py (fuente de verdad)
    2. Construye modelo ConvNeXt-Tiny desde cero (weights=None, 14 salidas)
    3. Entrena con BCEWithLogitsLoss(pos_weight) + FP16 + gradient accumulation
    4. Métricas: AUC-ROC por clase + F1 Macro + AUPRC (NUNCA Accuracy)
    5. Early stopping por val_loss (patience=10)
...
Dependencias:
    ...
    - src/pipeline/fase2/dataloader_expert1.py: build_dataloaders_expert1
...
from fase2.dataloader_expert1 import build_dataloaders_expert1
from fase2.expert1_config import (
    ...
    EXPERT1_EARLY_STOPPING_MONITOR,
...
def train_one_epoch(
    ...
    for batch_idx, (imgs, labels, _stems) in enumerate(loader):
        ...
        labels = labels.to(device, non_blocking=True)  # already float32 [B, 14]
...
def validate(
    ...
    Métricas computadas:
        ...
        - val_f1_macro: F1 Macro (threshold=0.5, multilabel)
        - val_auc: media de AUC-ROC por clase (14 clases)
        - val_auprc: media de AUPRC por clase (14 clases)

    Returns:
        dict con keys: val_loss, val_f1_macro, val_auc, val_auprc
    """
    model.eval()
    total_loss = 0.0
    n_batches = 0
    ...
    all_logits = torch.cat(all_logits, dim=0)  # [N, 14]
    all_labels = torch.cat(all_labels, dim=0).numpy().astype(int)  # [N, 14]

    probs = torch.sigmoid(all_logits).numpy()  # [N, 14]
    preds = (probs >= 0.5).astype(int)  # [N, 14]

    # ── AUC-ROC por clase (skip clases con una sola label) ─────────
    auc_per_class = []
    for c in range(14):
        try:
            auc_per_class.append(roc_auc_score(all_labels[:, c], probs[:, c]))
        except ValueError:
            auc_per_class.append(0.0)
    mean_auc = np.mean(auc_per_class)

    # ── F1 Macro (threshold en 0.5) ────────────────────────────────
    ...
    for c in range(14):
        try:
            auprc_per_class.append(
                average_precision_score(all_labels[:, c], probs[:, c])
            )
        except ValueError:
...
def train(dry_run: bool = False, data_root: str | None = None) -> None:
    """
    Función principal de entrenamiento del Expert 1.
    ...
    model = Expert1ConvNeXtTiny(
        fc_dropout_p=EXPERT1_DROPOUT_FC,
        num_classes=14,
    ).to(device)

    n_params = model.count_parameters()
    log.info(
        f"[Expert1] Modelo ConvNeXt-Tiny creado: {n_params:,} parámetros entrenables"
    )
    ...
    # Construir kwargs para dataloader si data_root fue proporcionado
    dl_kwargs = {
    ...
    if data_root is not None:
        root = Path(data_root)
        dl_kwargs["csv_path"] = (
            root / "datasets" / "nih-chest-xrays" / "Data_Entry_2017.csv"
        )
        dl_kwargs["img_dir"] = root / "datasets" / "nih-chest-xrays" / "all_images"
        dl_kwargs["train_list"] = root / "splits" / "nih_train.txt"
        dl_kwargs["val_list"] = root / "splits" / "nih_val.txt"
        dl_kwargs["test_list"] = root / "splits" / "nih_test.txt"
    ...
    log.info(f"[Expert1] Optimizer: AdamW(lr={EXPERT1_LR}, wd={EXPERT1_WEIGHT_DECAY})")
    ...
    log.info(
        "[Expert1] Scheduler: CosineAnnealingWarmRestarts(T_0=15, T_mult=2, eta_min=1e-6)"
    )
    ...
    log.info(
        f"[Expert1] EarlyStopping: monitor={EXPERT1_EARLY_STOPPING_MONITOR}, "
    ...
    training_log = []
    max_epochs = 1 if dry_run else EXPERT1_MAX_EPOCHS

    log.info(f"\n{'=' * 70}")
    log.info(f"  INICIO DE ENTRENAMIENTO — Expert 1 (ConvNeXt-Tiny / ChestXray14)")
    log.info(
    ...
    for epoch in range(max_epochs):
        ...
        val_auprc = val_results["val_auprc"]
        ...
        if not dry_run:
            if early_stopping.step(val_loss):
                log.info(
                    ...
                    f"val_loss no mejoró en {EXPERT1_EARLY_STOPPING_PATIENCE} épocas. "
    ...
    # ── Resumen final ──────────────────────────────────────────────
    log.info(f"\n{'=' * 70}")
    log.info(f"  ENTRENAMIENTO FINALIZADO — Expert 1 (ConvNeXt-Tiny / ChestXray14)")
    ...
    if training_log:
        ...
        log.info(
            ...
            f"AUPRC: {best_epoch['val_auprc']:.4f}"
        )
    if not dry_run:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Entrenamiento Expert 1 — ConvNeXt-Tiny / ChestXray14"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
