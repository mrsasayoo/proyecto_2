...
def extract_embeddings(model, dataloader, device, d_model, desc=""):
    ...
    log.info("[Extract/%s] Iniciando: %s imágenes | d_model=%d | batch=%d",
             desc, f"{n_total:,}", d_model, dataloader.batch_size)
    ...
    for batch_idx, (imgs, experts, names) in enumerate(dataloader):
        ...
        if np.isnan(z_np).any():
            nan_batches += 1
            log.error("[Extract/%s] NaN en batch %d (imgs %d–%d)",
                      desc, batch_idx, cursor, cursor + B)
        if np.isinf(z_np).any():
            inf_batches += 1
            log.error("[Extract/%s] Inf en batch %d (imgs %d–%d)",
                      desc, batch_idx, cursor, cursor + B)
        ...
        if batch_idx % 50 == 0:
            ...
            log.info("[Extract/%s] %6s/%s (%.1f%%) | %.0f img/s | ETA %.1f min",
                     desc, f"{cursor:,}", f"{n_total:,}",
                     100 * cursor / n_total, speed, eta_s / 60)
        ...
        if device == "cuda" and batch_idx % 200 == 0:
            ...
            log.debug("[Extract/%s] VRAM: %.2f/%.1f GB (%.1f%%)",
                      desc, used_gb, total_gb, pct)
            if pct > 90:
                log.warning("[Extract/%s] VRAM al %.1f%% — riesgo de OOM. "
                            "Considera reducir --batch_size.", desc, pct)

    elapsed_total = time.time() - t_start
    log.info("[Extract/%s] Completado: %s embeddings en %.1fs (%.0f img/s)",
             desc, f"{cursor:,}", elapsed_total,
             cursor / elapsed_total if elapsed_total > 0 else 0)

    # Reporte de anomalías
    if nan_batches or inf_batches:
        log.error("[Extract/%s] ANOMALÍAS: NaN en %d batches | Inf en %d batches. "
                  "Los embeddings no son confiables.", desc, nan_batches, inf_batches)
    else:
        log.info("[Extract/%s] Sin NaN ni Inf detectados ✓", desc)

    # Verificación de normas L2
    norms = np.linalg.norm(Z[:cursor], axis=1)
    log.debug("[Extract/%s] Norma L2 — media: %.2f | min: %.2f | max: %.2f",
              desc, norms.mean(), norms.min(), norms.max())
    if norms.mean() < MIN_L2_NORM:
        log.warning("[Extract/%s] Norma L2 media muy baja (%.3f < %.1f). "
                    "¿El backbone devuelve el CLS token correctamente?",
                    desc, norms.mean(), MIN_L2_NORM)
