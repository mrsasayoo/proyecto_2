...
sys.path.insert(0, str(PROJECT_ROOT / "fase0"))
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            str(LOGS_DIR / "fase0_pipeline.log"), mode="w",
        ),
    ],
)
...
def paso1_descargar(active, luna_subsets=None, dry_run=False):
    ...
    try:
        ...
        detail = run_downloads(DATASETS_DIR, active, luna_subsets=luna_subsets,
                               dry_run=dry_run)
...
def paso2_extraer(active, disco=False, luna_subsets=None, dry_run=False):
    ...
    try:
        ...
        detail = run_extractions(DATASETS_DIR, active, luna_subsets=luna_subsets,
                                 disco=disco)
        all_ok = all(v for v in detail.values()) if detail else True
        return {"status": "✅" if all_ok else "⚠️", "detail": detail}
    except Exception as e:
...
def paso4_pancreas_labels(active, dry_run=False):
    ...
    if out_csv.exists() and out_csv.stat().st_size > 100:
        ...
        if len(valid) > 10:
            log.info("  pancreas_labels_binary.csv ya existe (%d filas, %d válidas)",
                     len(df_check), len(valid))
    ...
    log.info("  PDAC+=%d  PDAC-=%d  unknown=%d  total=%d",
             pdac_pos, pdac_neg, unknown, len(df))
    return {
        "status": "✅",
        "pdac_pos": pdac_pos,
        "pdac_neg": pdac_neg,
...
def paso6_pre_embeddings(active, workers=6, neg_ratio=10, max_neg=None,
    ...
    try:
        ...
        return run_pre_embeddings(
            DATASETS_DIR, active, workers=workers, neg_ratio=neg_ratio,
...
def main():
    ...
    parser.add_argument(
        "--solo_pasos", nargs="+", type=int, default=None,
        help="Solo ejecutar pasos específicos (0-8)",
    )
    parser.add_argument(
        "--disco", action="store_true",
        help="Modo disco — borrar ZIP tras extracción",
    )
    parser.add_argument(
        "--luna_subsets", nargs="+", type=int, default=None,
    ...
    def run_paso(num, func, *func_args, **func_kwargs):
        ...
        timings[key] = elapsed
        log.info("Paso %d completado en %.1fs — %s",
                 num, elapsed, result.get("status", "?"))
    ...
    run_paso(6, paso6_pre_embeddings, active, args.workers, args.neg_ratio,
             args.max_neg, args.luna_subsets, args.dry_run)
