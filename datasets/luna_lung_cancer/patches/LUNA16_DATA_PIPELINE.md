...
# For 12,616 neg / 1,263 pos: aug_per_pos = 3
...
- **Disk space:** The augmented set is ~27% larger than the original training set (17,668 vs. 13,879 files).
...
**Note on `train_aug` positives:** 5,052 = 1,263 originals + 3,789 augmented copies (3 augmented per original). The 12,616 negatives match `train/` exactly — no corrupt patches were excluded during the augmentation process (see `train_aug_report.json`: `corrupt_files: []`).
...
|---|---|---|
| `train` | 13,879 | ~13.2 GB |
| `val` | 1,155 | ~1.1 GB |
| `test` | 2,013 | ~1.9 GB |
| `train_aug` | 17,668 | ~16.9 GB |
| **Total** | **34,715** | **~33.1 GB** |
